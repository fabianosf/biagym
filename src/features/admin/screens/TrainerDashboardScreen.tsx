...
const SHORTCUTS: { icon: keyof typeof Ionicons.glyphMap; labelKey: string; href: Href }[] = [
  { icon: 'people-outline', labelKey: 'admin.dashboard.shortcutStudents', href: adminRoutes.students as Href },
  // Mesma tela da aba "Treinos" da barra inferior — o atalho só troca de aba, não empilha rota.
  { icon: 'barbell-outline', labelKey: 'admin.dashboard.shortcutWorkouts', href: '/workouts' as Href },
  // Mesma tela da aba "Programas" da barra inferior.
  { icon: 'play-circle-outline', labelKey: 'admin.dashboard.shortcutPrograms', href: '/store' as Href },
  { icon: 'chatbubbles-outline', labelKey: 'admin.dashboard.shortcutMessages', href: adminRoutes.messages as Href },
];
  ...
  return (
    <View className="flex-1 bg-background">
      <View className="px-5 pb-2" style={{ paddingTop: insets.top + 8 }}>
        <Text className="text-sm text-muted">{formatHelloGreeting(t, user?.name)}</Text>
        <Text className="mt-1 text-[26px] font-bold text-ink">{t('admin.dashboard.title')}</Text>
      </View>
      <OfflineBanner />

      {isLoading ? (
        <LoadingIndicator fullScreen message={t('admin.dashboard.loading')} />
      ) : (
        <ScrollView
          className="flex-1"
        ...
        >
          {activities.length === 0 ? (
            <EmptyState
              ...
              title={t('admin.dashboard.noStudentsTitle')}
              description={t('admin.dashboard.noStudentsDescription')}
            />
          ) : (
            <>
              <View className="flex-row gap-3">
                ...
                <MetricCard
                  ...
                  label={t('admin.dashboard.inactiveDays')}
              ...
              <QuietStudentsList title={t('admin.dashboard.needsAttention')} items={quiet} limit={5} variant="days" />
            </>
          )}
        </ScrollView>
      )}
